...
class Storage:
    ...
    def __init__(self, max_changes: int=1,host="localhost", port=8000):
        ...
        self.collection = self.chroma_client.get_or_create_collection("news", embedding_function=hf_embeddings_function, metadata={"hnsw:space": "cosine"})
        os.makedirs(self.storage_path, exist_ok=True)
        self.df = pl.DataFrame()
        if os.path.exists(f"{self.storage_path}/data.parquet"):
            self.df = pl.read_parquet(f"{self.storage_path}/data.parquet") # да надо по нормальному но мне лень 

    def embed_documents_to_collection(self, documents, embedding_function):
        ids = [doc["metadata"]["id"] for doc in documents]

        if ids:
            existing_documents = self.collection.get(ids=ids, include=["metadatas"])["metadatas"]
        else:
            existing_documents = []
        ...
        self.collection.add(ids=ids_new, documents=documents_new, metadatas=metadatas_new, embeddings=embeddings)
    ...
    def get_rss(self, source: SOURCE):
        ...
        df = (
            ...
            .with_columns(
                ...
                pl.col("text").map_elements(preprocess_text_b25, return_dtype=pl.Utf8).alias("text_cleaned_b25"),
            )
        )
        if self.df.is_empty():
    ...
    def search_chroma_core(self,query: str, dates: Optional[List[date]], sources: Optional[List[str]], n: int) -> List[SearchResult]:
        if self.df.is_empty():
            return []
    ...
    def search_bm_25_core(self,query: str, dates: Optional[List[date]], sources: Optional[List[str]], top_k: int=10) -> List[SearchResult]:
        if self.df.is_empty():
            return []
